...
from tleague.actors.agent import GAILAgent
...
class GAIL2Actor(VtraceActor):
  ...
  def __init__(self, env, policy, league_mgr_addr, model_pool_addrs,
    ...
    super(GAIL2Actor, self).__init__(env, policy, league_mgr_addr,
                                     model_pool_addrs,
  ...
  def _rollout_an_episode(self):
    ...
    while True:
      ...
      if self._enable_push:
        ...
        # data_queue, which is watched in another Thread (_push_data_to_learner
        ...
        rwd_to_push = np.concatenate([rwd_to_push, disc_reward])
      ...
      if self._update_model_freq and \
          self._steps % self._update_model_freq == 0:
  ...
  def _push_data_to_learner(self, data_queue):
    ...
    while True:
      ...
      unroll_gail = []
      infos = []
      mask = False  # For the first frame in a unroll, there is no need to care
      # about whether it is a start of a new episode, because even if it is
      # new start, hidden state is zero and this is equivalent to mask=True.
      # For other cases, mask must be False. So, just set mask=False here.
      while True:
        if last_obs[me_id] is not None:
          ...
          data_pg.r = reward
          data_pg.discount = np.asarray((1 - done) * self._gamma, np.float32)
          ...
          data_pg.r += (self._gamma ** mb_skips[-1]) * reward
          data_pg.discount *= np.asarray((1 - done) * self._gamma, np.float32)
